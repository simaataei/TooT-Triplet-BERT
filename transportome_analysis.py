...
fasta_file = 'Dataset/Ecoli-K12-proteins.fasta'
file_name = 'Ecoli-K12-proteins'
...
print('sequence is this:'+sequences[2])
#testing with KNN
...
with open(f'Results/{file_name}_resutls_offline_e97.txt', 'w') as file:
    ...
    for name, label in zip(seq_name, predictions):
        ...
        label_name_l = label_name.get(label, "Unknown Label Name")
        label_name_list.append(label_name_l)   
     
        chebi_id_l = label_chebi.get(label, "Unknown ChEBI ID")
        chebi_id_list.append(chebi_id_l)
        # Write to file
        file.write(f"{name}, {label}, {chebi_id_l}, {label_name_l}\n")
...
print("Name-Distances:\n", name_distance)
df = pd.DataFrame({'neighbours':name_distance})
with open(f'./Results/{file_name}_offline_sampling_up_100_e97_3nn_test_train.txt', 'w') as f:
        with pd.option_context("max_colwidth", 1000):
            f.write(df.to_latex(index=True))
...
print(df)
latex_code = df.to_latex(index=False, caption='Neighbor Details', label='tab:neighbor_details', longtable=True)
with open(f'Results/3neighbor_details_{file_name}_offline_e97_test_train.tex', 'w') as file:
    file.write(latex_code)
